...
parser.add_argument('-f', '--file', help="Follow users from a File pre-generated")
parser.add_argument('-mf', '--max-followers', help="Max Number of Followers to Follow")
    ...
    for page in tqdm.tqdm(range(1, int(last_page)), ncols=35, smoothing=True, bar_format='[PROGRESS] {n_fmt}/{total_fmt} | {bar}'):
        ...
        if mnof != None:
            if len(users_to_follow) >= int(mnof):
                break
    filename = str(datetime.now().strftime('%m-%d-%YT%H-%M-%S')) + "-" + str(len(users_to_follow)) + "_FOLLOWED.json"
